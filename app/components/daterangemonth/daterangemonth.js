...
$(document).ready(function () {
    var AllTimeDateRange, ArrayUtils, CalendarHeaderView, CalendarView, Config, CustomDateRange, DateRange, daterangemonthView, MomentIterator, MomentUtil, Period,
    ...
    MomentIterator = (function () {
        MomentIterator.array = function (date, amount, period) {
            let i, iterator, j, ref, results;
            iterator = new this(date, period);
            results = [];
            for (i = j = 0, ref = amount - 1; ref >= 0 ? j <= ref : j >= ref; i = ref >= 0 ? ++j : --j) {
    ...
    $.fn.daterangemonth = function (options, callback) {
        if (options == null) {
            options = {};
        }
        this.each(function () {
            let $element;
            $element = $(this);
            if (!$element.data('daterangemonth')) {
                options.anchorElement = $element;
                if (callback) {
                    options.callback = callback;
                }
                options.callback = $.proxy(options.callback, this);
                return $element.data('daterangemonth', new daterangemonthView(options));
            }
        });
        return this;
    };
    ...
    ko.bindingHandlers.daterangemonth = (function () {
        return $.extend(this, {
            _optionsKey: 'daterangemonthOptions',
            _formatKey: 'daterangemonthFormat',
            init: function (element, valueAccessor, allBindings) {
                let observable, options;
                observable = valueAccessor();
                options = ko.unwrap(allBindings.get(this._optionsKey)) || {};
                return $(element).daterangemonth(options, function (startDate, endDate, period) {
                    return observable([startDate, endDate]);
                });
            },
            update: function (element, valueAccessor, allBindings) {
                let $element, dateFormat, endDate, endDateText, ref, startDate, startDateText;
                $element = $(element);
                ...
                return ko.ignoreDependencies(function () {
                    let text;
                    if (!$element.data('daterangemonth').standalone()) {
                        text = $element.data('daterangemonth').single() ? startDateText : startDateText + ' – ' + endDateText;
                        $element.val(text).text(text);
                    }
                    $element.data('daterangemonth').startDate(startDate);
                    return $element.data('daterangemonth').endDate(endDate);
                });
            }
        });
    })();
    ...
    Config = (function () {
        ...
        Config.prototype._dateObservable = function (val, mode, minBoundary, maxBoundary) {
            let computed, fitMax, fitMin, observable;
            ...
            computed.isWithinBoundaries = (function (_this) {
                return function (date) {
                    let between, max, maxExclusive, min, minExclusive, sameMax, sameMin;
    ...
    CalendarHeaderView = (function () {
        ...
        CalendarHeaderView.prototype.monthOptions = function () {
            let j, maxMonth, minMonth, results;
    ...
    CalendarView = (function () {
        ...
        CalendarView.prototype.calendar = function () {
            let col, cols, date, iterator, j, ref, ref1, results, row, rows;
            ref = this.period.dimentions(), cols = ref[0], rows = ref[1];
            iterator = new MomentIterator(this.firstDate(), this.period());
            results = [];
            for (row = j = 1, ref1 = rows; ref1 >= 1 ? j <= ref1 : j >= ref1; row = ref1 >= 1 ? ++j : --j) {
                results.push((function () {
                    let l, ref2, results1;
                    results1 = [];
                    for (col = l = 1, ref2 = cols; ref2 >= 1 ? l <= ref2 : l >= ref2; col = ref2 >= 1 ? ++l : --l) {
                        date = iterator.next();
                        if (this.type === 'end') {
        ...
        CalendarView.prototype.cssForDate = function (date, periodIsDay) {
            let differentMonth, inRange, obj1, onRangeEnd, withinBoundaries;
        ...
        CalendarView.prototype.firstYearOfDecade = function (date) {
            let currentYear, firstYear, offset, year;
    ...
    daterangemonthView = (function () {
        function daterangemonthView(options) {
            let endDate, ref, startDate, wrapper;
            if (options == null) {
                options = {};
            }
            ...
            if (this.anchorElement) {
                ...
                if (!this.standalone()) {
                    $(document).on('mousedown.daterangemonth', this.outsideClick).on('touchend.daterangemonth', this.outsideClick).on('click.daterangemonth', '[data-toggle=dropdown]', this.outsideClick).on('focusin.daterangemonth', this.outsideClick);
                }
            }
            if (this.opened()) {
                this.updatePosition();
        ...
        daterangemonthView.prototype.periodProxy = Period;

        daterangemonthView.prototype.calendars = function () {
            if (this.single()) {
        ...
        daterangemonthView.prototype.updateDateRange = function () {
            return this.dateRange([this.startDate(), this.endDate()]);
        };

        daterangemonthView.prototype.cssClasses = function () {
            let j, len, obj, period, ref;
            obj = {
                single: this.single(),
        ...
        daterangemonthView.prototype.isActivePeriod = function (period) {
            return this.period() === period;
        };

        daterangemonthView.prototype.isActiveDateRange = function (dateRange) {
            let dr, j, len, ref;
        ...
        daterangemonthView.prototype.isActiveCustomPeriodRange = function (customPeriodRange) {
            return this.isActiveDateRange(customPeriodRange) && this.isCustomPeriodRangeActive();
        };

        daterangemonthView.prototype.inputFocus = function () {
            return this.expanded(true);
        };

        daterangemonthView.prototype.setPeriod = function (period) {
            this.isCustomPeriodRangeActive(false);
            this.period(period);
            return this.expanded(true);
        };

        daterangemonthView.prototype.setDateRange = function (dateRange) {
            if (dateRange.constructor === CustomDateRange) {
                return this.expanded(true);
            } else {
                this.expanded(false);
        ...
        daterangemonthView.prototype.setCustomPeriodRange = function (customPeriodRange) {
            this.isCustomPeriodRangeActive(true);
            return this.setDateRange(customPeriodRange);
        };

        daterangemonthView.prototype.applyChanges = function () {
            this.close();
            return this.updateDateRange();
        };

        daterangemonthView.prototype.cancelChanges = function () {
            return this.close();
        };

        daterangemonthView.prototype.open = function () {
            return this.opened(true);
        };

        daterangemonthView.prototype.close = function () {
            if (!this.standalone()) {
                return this.opened(false);
            }
        };

        daterangemonthView.prototype.toggle = function () {
            if (this.opened()) {
                return this.close();
            } else {
                return this.open();
        ...
        daterangemonthView.prototype.updatePosition = function () {
            let parentOffset, parentRightEdge, style;
            if (this.standalone()) {
                return;
            }
            parentOffset = {
        ...
        daterangemonthView.prototype.outsideClick = function (event) {
            let target;
            target = $(event.target);
    ...
    $.extend($.fn.daterangemonth, {
        ...
        daterangemonthView: daterangemonthView,
