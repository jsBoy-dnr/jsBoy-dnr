...
var polls_item = {
    user_info: {
        photo_url: '../../images/happy_b_day_image14.png',
        fio: 'Васюкович Юлия Анатольевна',
        position: 'Директор по<br />подбору персонала',
    ...
    },
    {
        ...
        items: [{
            ...
            txt: '4. Я готов рекомендовать Компанию СУЭК как стабильного и ответственного работодателя своим друзьям и знакомым',
            items: ['Полностью НЕ согласен', 'Скорее НЕ согласен', 'Скорее согласен', 'Полностью согласен']
        },
        {
            id: 2,
            type: 'single_radio_list_horizontal',
            txt: '5. Я горжусь, что работаю в компании, которая является крупнейшим производителем угля в России',
            items: ['Полностью НЕ согласен', 'Скорее НЕ согласен', 'Скорее согласен', 'Полностью согласен']
        },
        {
            id: 3,
            type: 'single_radio_list_horizontal',
            txt: '6. Я горжусь, что работаю в компании, которая имеет самое современное оборудование',
            items: ['Полностью НЕ согласен', 'Скорее НЕ согласен', 'Скорее согласен', 'Полностью согласен']
        },
        {
            id: 4,
            type: 'single_radio_list_horizontal',
            txt: '7. Я горжусь, что работаю в компании, которая стабильно выплачивает заработную плату, предоставляет социальные льготы работникам',
            items: ['Полностью НЕ согласен', 'Скорее НЕ согласен', 'Скорее согласен', 'Полностью согласен']
        }
        ]
    },
...
function checkMultiRow_v2(dom_obj) {
    ...
    $('.poll_single_radio_list_vertical_item_list').each(function () {
        ...
        $(this).find('.poll_single_radio_list_vertical_item_list_radio').each(function () {
            ...
            var el = $('#poll_single_radio_list_vertical_item_input' + $(this).data('input_id'));
...
function onClickMultiRow_2(dom_obj, need_input) {
    ...
    $(dom_obj).parent().find('.poll_single_radio_list_vertical_item_list_radio > img').attr('src', '../../images/icons/radiobtn-uncheck.svg');
    $(dom_obj).parent().find('.poll_single_radio_list_vertical_item_input').hide();

    $(dom_obj).find('img').attr('src', '../../images/icons/radiobtn-check.svg');
    $(dom_obj).data('status', 'checked');

    if (need_input === true) {
        var el = $('#poll_single_radio_list_vertical_item_input' + $(dom_obj).data('input_id'));
        el.show();
    }
...
function pollSingleCheckboxSelector_Check(dom_obj, minimal_count_check) {
    ...
    $(dom_obj).parent().find('.poll_single_checkbox_selector_item').each(function () {
        ...
        if ($(this).data('status') === 'checked') {
            var el = $('#poll_single_checkbox_selector_item_input' + $(this).data('input_id'));
...
function pollClickCheckBoxBnt_1(dom_obj, need_input, minimal_count_check) {
    if ($(dom_obj).data('status') === 'unchecked') {
        ...
        if (need_input === true) {
            var el = $('#poll_single_checkbox_selector_item_input' + $(dom_obj).data('input_id'));
            el.show();
        }
    } else {
...
function onClickMultiRow_1(dom_obj, _count) {
    $(dom_obj).parent().find('.poll_single_radio_list_horizontal_item_list_radio').data('status', 'unchecked');
    $(dom_obj).parent().find('.poll_single_radio_list_horizontal_item_list_radio > img').attr('src', '../../images/icons/radiobtn-uncheck.svg');
...
function pollClickRadioBnt_1(dom_obj, need_input) {
    $(dom_obj).parent().find('.poll_single_radio_selector_item').data('status', 'unchecked');
    $(dom_obj).parent().find('.poll_single_radio_selector_item > img').attr('src', '../../images/icons/radiobtn-uncheck.svg');
    ...
    if (need_input === true) {
        var el = $('#poll_single_radio_selector_item_input' + $(dom_obj).data('input_id'));
        el.show();

        onKeyUpRadioBnt_1(el.find('input'));
    } else {
        $(dom_obj).parent().find('.poll_page_buttom').prop('disabled', false);
    }
}
...
function saveCurrentPoll() {
    ...
    if (obj.type === 'single_radio_selector') {
        ...
        var txt = $.trim($('.poll_input_value > input').val());
...
function buildPoll(index) {
    ...
    if (obj.type === 'first') {
        ...
        ss += '<div class="poll_page_buttom_c"><button class="poll_page_buttom" onclick="buildPoll(' + (_index + 1) + ')">Начать</button>';
        ...
        for (var tt in obj.items) {
            ...
            ss += '<img src="../../images/icons/radiobtn-uncheck.svg" />';
            ss += '<span>' + tt.txt + '</span>';
            ss += '</div>';
        ...
        ss += '<div class="poll_page_buttom_c"><button class="poll_page_buttom" disabled="disabled" onclick="saveCurrentPoll();buildPoll(' + (_index + 1) + ')">Далее</button>';
        ss += '<div class="poll_page_menu_footer" onclick="openCloseFooterUserInfo(this)">';
        ss += '<span>Есть вопросы?</span><img src="../../images/icons/arrow-down-gold.svg" />';
        ss += '</div>';
        ss += '</div>';
        ...
        for (var tt in obj.items) {
            if (tt.type === 'single_radio_list_horizontal') {
                ss += '<div class="poll_single_radio_list_horizontal_item">';
        ...
        ss += '<div class="poll_page_buttom_c"><button class="poll_page_buttom">Архив опросов</button>';
        ss += '<div class="poll_page_menu_footer" onclick="openCloseFooterUserInfo(this)">';
        ss += '<span>Есть вопросы?</span><img src="../../images/icons/arrow-down-gold.svg" />';
        ss += '</div>';
        ss += '</div>';
        ...
        if (obj.chart.length > 0) {
            ...
            var max = obj.chart[0][0];

            for (var _h in obj.chart) {
                if (max < _h[0]) {
                    max = _h[0];
                }
            }
