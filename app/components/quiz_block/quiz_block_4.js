var polls_item = {
    ...
    items: [{
        ...
        },
        {
            ...
            title: 'Прочитайте внимательно вопрос и выберите один из вариантов ответа',
            text: '<p>Вы предоставляли от имени (или) в интересах Общества или иного юридического лица Группы денежные средства и (или) иное имущество, и (или) иную имущественную выгоду (напрямую или через других лиц) государственным или муниципальным служащим, в избирательные фонды кандидатов, избирательных объединений за осуществление такими лицами их полномочии вопреки законным интересам общества и государства? </p>',
            items: [{
                    id: 1,
                    txt: 'Да',
                    need_input: true,
...
// Эта часть повторяется во всех quiz_block_.js
...
function buildPoll(index) {
    ...
    if (obj.type === 'first') {
        ...
        ss += '</div>';
        if (obj.text) {
            ss += '<div class="poll_page_left_text">';
            ss += obj.text;
            ss += '</div>';
        }
        ...
        ss += '<div class="poll_page_buttom_c">'
        ss += '<button class="poll_page_buttom btn btn-outline-primary" onclick="buildPoll(' + (_index - 1) + ')">Назад</button>'
        ss += '<button class="poll_page_buttom btn btn-primary" disabled="disabled" onclick="saveCurrentPoll();buildPoll(' + (_index + 1) + ')">Далее</button>';
        ss += '<div class="poll_page_menu_footer" onclick="openCloseFooterUserInfo(this)">';
        ss += '<span>Есть вопросы?</span><img src="../images/icons/arrow-down-gold.svg" />';
        ss += '</div>';
        ss += '</div>';
