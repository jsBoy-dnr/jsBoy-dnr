'use strict'; var Monthpicker = function () {
    ...
    function c(a, b) { this.selectedMonth = this.selectedYear = this.currentYear = null; this.id = c.next_id++; c.instances[this.id] = this; this.original_input = a; this.InitOptions(b); this.InitValue(); this.Init(); this.RefreshInputs() } c.Get = function (a) { if ("undefined" === typeof a.parentElement.dataset.mp) throw "Unable to retrieve the Monthpicker of element " + a; return c.instances[a.parentElement.dataset.mp] }; c.prototype.InitValue = function () {
        var a = new Date; this.currentYear = a.getFullYear(); this.currentMonth = a.getMonth();
        ...
        monthNames[this.currentMonth] + ", " + this.currentYear.toString()  + "</span>"
        ...
        this.parent = document.createElement("div"); this.parent.classList.add("monthpicker"); this.parent.tabIndex = -1; var a = getComputedStyle(this.original_input, null); this.original_input.parentElement.insertBefore(this.parent, this.original_input); this.parent.appendChild(this.original_input); this.original_input.style.display = "none"; this.input = document.createElement("div"); this.input.classList.add("monthpicker_input"); this.input.classList.remove('opened'); this.parent.appendChild(this.input); this.selector = document.createElement("div"); this.selector.classList.add("monthpicker_selector"); this.selector.style.display = "none"; for (var a = "<table><tr class='t-header'><td><span class='yearSwitch down'></span></td><td><div class='yearValue'>" + this.currentYear + "</div> </td><td><span class='yearSwitch up'></span> </td></tr> ", b = 0; 4 > b; b++)var c = 3 * b, d = this.opts.monthLabels.slice(c,
                    c + 3), a = a + ("<tr><td class='month month" + (c + 1) + "' data-m='" + (c + 1) + "'>" + d[0] + "</td><td class='month month" + (c + 2) + "' data-m='" + (c + 2) + "'>" + d[1] + "</td><td class='month month" + (c + 3) + "' data-m='" + (c + 3) + "'>" + d[2] + "</td></tr>"); this.selector.innerHTML = a + "</table>"; this.parent.appendChild(this.selector)
    }; c.prototype.Init = function () {
        this.InitIU(); this.year_input = this.selector.querySelector(".yearValue"); this.parent.dataset.mp = this.id.toString(); this.parent.addEventListener("focus", function () { c.instances[this.dataset.mp].Show() },
        ...
        }; c.prototype.PrevYear = function () { this.ShowYear(this.currentYear - 1) }; c.prototype.NextYear = function () { this.ShowYear(this.currentYear + 1) }; c._clone = function (a) {
        ...
        }; c.next_id = 1; c.instances = []; c.defaultOpts = { minValue: null, minYear: null, maxValue: null, maxYear: null, monthLabels: "Январь Февраль Март Апрель Май Июнь Июль Август Сентябрь Октябрь Ноябрь Декабрь".split(" "), onSelect: null, onClose: null, allowNull: !0 }; return c
}();
...
}); window.Element && function (c) { c.matches = c.matches || c.matchesSelector || c.webkitMatchesSelector || c.msMatchesSelector || function (a) { a = (this.parentNode || this.document).querySelectorAll(a); for (var b = -1; a[++b] && a[b] != this;); return !!a[b] } }(window.Element.prototype);
